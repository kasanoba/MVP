...
def convert_cobol_code(cobol_code: str, target_lang: str) -> str:
    ...
    prompt = (
        f"다음 입력은 COBOL 코드입니다. {target_lang.capitalize()} 코드로 변환해주세요.\n"
        "만약 입력된 텍스트가 유효한 COBOL 코드가 아니라면, '해당 입력은 COBOL 코드가 아닙니다. \nCOBOL 코드만 변환이 가능합니다.'라고만 응답하세요.\n"
        "변환 시, 반드시 전체를 하나의 텍스트 블록으로, 코드 블록 마크다운(```) 없이 출력하세요.\n"
        "변환된 코드에는 필요한 최소한의 주석만 추가하고, 기타 설명은 모두 주석으로 처리하세요.\n\n"
        f"COBOL 코드:\n{cobol_code}\n"
    )
    try:
        # OpenAI Chat API 호출
